# NovaOpsTest Tools — LEDStock API (DEV)

> ⚠️ **THIS AGENT ONLY USES THE DEV-NLWSTOCK API.**
> `BASE` is hardcoded to `https://dev-stock.nlwzhc.com/api`.
> Never change this to production. Never call any other API endpoint.
...
```bash
BASE="https://dev-stock.nlwzhc.com/api"
...
```
⚠️ Approval requires Lars or Daniel — NovaOpsTest cannot approve.
